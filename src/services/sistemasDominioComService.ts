...
export interface SistemaDominioComRegistro {
  id: number;
  module_id: number;
  user_id: number;
...
export const sistemasDominioComService = {
  async checkAvailability(dominioNome: string) {
    ...
    return apiRequest<{
      ...
      dominio_completo: string;
      disponivel: boolean;
      registro: SistemaDominioComRegistro | null;
    }>(`/sistemas-dominio-com/check?${qs.toString()}`);
  },
  ...
  async deleteByAdmin(id: number) {
    return apiRequest<{ id: number; refunded_amount?: number }>(`/sistemas-dominio-com/${id}`, {
      method: 'DELETE',
    });
  },
