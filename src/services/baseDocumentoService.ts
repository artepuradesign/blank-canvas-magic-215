 ...
 export interface CreateBaseDocumento {
   cpf_id: number;
   numero_identificador?: string;
   data_expedicao?: string;
   orgao_emissor?: string;
 ...
 async function apiRequest<T>(endpoint: string, options: RequestInit = {}): Promise<ApiResponse<T>> {
   try {
     ...
     if (!sessionToken) {
       console.error('❌ [BASE_DOCUMENTO_API] Token de sessão não encontrado');
       return {
         success: false,
     ...
     console.error('❌ [BASE_DOCUMENTO_API] Request Error:', error);
     return {
       success: false,
 ...
 export const baseDocumentoService = {
   async getByCpfId(cpfId: number) {
     return apiRequest<BaseDocumento | null>(`/base-documento/cpf/${cpfId}`);
   },

   async create(data: CreateBaseDocumento) {
     return apiRequest<{ id: number }>('/base-documento', {
       method: 'POST',
